...
import { saveRetroHistory, retroHistoryKey, type RetroHistoryScheduler } from "./retro-history";

export function createRetroHistoryWriter(options: {
  debounceMs?: number;
  ...
  const flush = (): boolean | null => {
    ...
    cancel();
    let success = false;
    try { success = save(current.room, current.viewerId); } catch { /* Collaboration is independent of storage. */ }
    if (success && current.room.phase === "closed") finalKey = retroHistoryKey(current.room);
  ...
  return {
    enqueue(room: RetroRoom, viewerId?: string | null) {
      ...
      if (room.phase === "closed") { flush(); return; }
      if (timer === undefined) timer = scheduler.setTimeout(() => { timer = undefined; flush(); }, options.debounceMs ?? 250);
    },
    flush,
    cancel,
    reset() { cancel(); finalKey = null; },
  };
}
