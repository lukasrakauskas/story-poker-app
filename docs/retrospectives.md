...
- Transient socket failures automatically reconnect with bounded exponential backoff and jitter, then send only a `resume` command. The first retry is normally about one second and the delay is capped at 30 seconds; a connection that remains healthy for 10 seconds resets the backoff. The card announces the current attempt and keeps a manual Retry action available.
- Browser `offline` pauses retry timers, `online` reconnects promptly, and a hidden tab pauses pending retries until it becomes visible. Expired, rejected, replaced, removed, or closed sessions do not auto-retry.
- If a mutation was sent but its acknowledgement was lost, the client resolves the action as uncertain, discards the command, refreshes the authoritative snapshot on resume, and keeps the warning visible. It never replays that mutation automatically; inspect the room before trying it again.
...
- `apps/frontend/hooks/use-retro-socket.ts`: isolated connection, cookie resume lifecycle, bounded recovery, and snapshot persistence.
- `apps/frontend/lib/retro-reconnect-policy.ts`: injectable bounded backoff/jitter policy used by the socket lifecycle.
