...
- Reconnection credentials are stored by the backend origin in one room-scoped HttpOnly, Secure cookie per room (`retro-session-<code>`, `Path=/retro`, `SameSite=None`). The frontend establishes, inspects, resumes, and forgets it through credentialed HTTP; it never reads `document.cookie`. Cookies expire with the room, not with the tab, and are never stored in localStorage. Reopening the room link restores the same identity, note ownership, votes, and moderator access while the live room exists and the identity remains within its offline retention window. Joining a remembered room by code also resumes that identity.
- HTTP resume rotates the cookie and displaces the prior socket. Removal, explicit forget, offline-member expiry, room expiry, and server restart revoke access. Failed resume/forget requests do not clear cookies, so an old socket or delayed response cannot erase a newer valid shared-browser cookie. Opening the same room in another tab remains last-connection-wins; see [`docs/retro-security.md`](retro-security.md) for the XSS, cross-site WebSocket, shared-device, replay, and separate-origin threat model.
...
- `apps/backend/src/retro/retro.gateway.ts`: the transport-only Nest controller for origin/cookie capture, runtime DTO validation, rate-limit delegation, heartbeat registration, application delegation, and response dispatch.
- `apps/backend/src/retro/retro-session.controller.ts` and `retro-session-cookie.service.ts`: the HTTP establishment, inspection, rotation, forget, and Set-Cookie boundary. Credentials never cross into shared room events.
- `apps/backend/src/transport`: shared WebSocket serialization/connection adapters, origin policy, configurable heartbeat handling, application event dispatch, and keyed throttling. `RetroModule` imports both this module and `CollaborationModule`.
- `apps/frontend/app/retro`: retrospective routes and UI, including the responsive room status card.
- `apps/frontend/hooks/use-retro-socket.ts`: isolated HTTP session establishment/rotation, cookie-authenticated connection lifecycle, and snapshot persistence.
- `apps/frontend/lib/retro-session.ts`: credentialed HTTP session helpers that deliberately never read or write the HttpOnly cookie.
...
The WebSocket/HTTP end-to-end tests exercise real clients, HttpOnly cookie establishment and rotation, room binding, origin rejection, private state, permissions, replay rejection, shared reconnect replacement, and coexistence with the original poker endpoint. Cross-domain contract tests prove that Poker and Retro receive the same normalized-name, unique-name, role, token, disconnect, and resume guarantees. Application-service tests cover session orchestration, recipient privacy, rotation, forget/removal/expiry revocation, audience events, and replacement without constructing sockets. Small gateway/transport contract tests cover cookie/origin boundaries, DTO validation, serialization, rate limiting, heartbeat cleanup, and protocol wiring; domain service tests cover phase transitions, voting budgets, resource limits, and domain-specific retention/expiry. Bun frontend unit tests cover credentialed HTTP helpers, no-JS-secret behavior, history updates, separate room lifetimes, corruption/quota handling, credential exclusion, and Markdown output. Browser regression covers separate-profile collaboration, request-specific acknowledgements (unrelated broadcasts cannot clear a pending draft), refresh/reopen identity and moderator recovery, same-profile tab replacement without cookie deletion races, stale sessions, all phases, saved final history without a live server/token, deletion, Markdown clipboard/download/fallback, connected/disconnected/expiring/storage-failure status, responsive status placement, and dark mode.
