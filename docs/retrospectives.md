...
4. **Arrange:** the moderator reveals notes and everyone may drag them between the Went well, To improve and Ideas lanes or stack messages that express the same idea. Stacks remain unnamed and can be expanded to show every original message. Live, color-coded pointers and card outlines show who is dragging; normalized pointer positions transition smoothly across different viewport sizes.
5. **Vote:** each participant has three selections across individual notes. Stacked messages remain visibly grouped, with a separate voting control for every original message. Only participants' own selections are visible until discussion; voter identities remain server-only.
6. **Discuss:** anonymous totals and consistent tied rankings are revealed. Stacks remain visibly grouped and are ranked by their aggregate votes while showing each original message's total. The moderator records actions, assigns/reassigns participant or external owners, and marks completion. Removed owners retain their name snapshots. Mobile actions remain reachable.
...
- Rooms expire two hours after creation, not after their last activity. Capacity is 100 rooms per repository namespace, 30 participants, 300 notes and 100 actions per room. Titles are bounded to 100 characters, note/action text to 1,000, external owners to 60.
