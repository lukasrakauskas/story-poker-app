...
- During private writing, only an author can delete their own note; moderators cannot inspect or target writing that was not sent to them. After reveal, the moderator can delete any note during voting or discussion with a destructive confirmation. Removing a note removes all votes attached to that note and releases those vote selections.
- A moderator can remove any other participant, including an offline participant, but cannot remove themselves. Removal immediately revokes resume access and closes an active socket. The removed participant's votes are discarded; retained notes keep a stable author-name snapshot, and free-text action owners remain readable even when they named that participant.
