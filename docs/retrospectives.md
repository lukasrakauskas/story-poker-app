...
- A valid room link first sends an unauthenticated `inspect` command after the WebSocket is ready when no saved credential can resume the room. The `retro-room-info` response contains only the requested code, join availability, and a password-required flag. Only active, non-full rooms are advertised as available; unknown, expired, and closed rooms remain opaque and do not render participant fields. The lobby offers a link to create or join another room. A saved credential skips inspection and uses the normal resume path.
